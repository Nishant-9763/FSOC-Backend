...
if(!isvalidpassword(password.trim())) return res.status(400).send({status: false,  mesage: "password length must be  8 to  15 char"});
...
const loginUser = async (req,res)=>{
   try {
    ...
    if(!isvalidpassword(password.trim())) return res.status(400).send({status: false,  message: "password length must be  8 to  15 char"});
